...
describe('ReservationController', () => {
  ...
  it('/reservation (GET)', async () => {
    ...
    const response = await request(app.getHttpServer())
        ...
        expect(response.body[0].reservationDate).toBe(reservation.reservationDate);
  ...
  it('/reservation/:id (GET)', async () => {
    ...
    const response = await request(app.getHttpServer())
        ...
        expect(response.body.reservationDate).toBe(reservation.reservationDate);
  ...
  it('/reservation (POST)', async () => {
    ...
    reservationDTO.identification = "1053847610";
    reservationDTO.name = "Test Name";
    reservationDTO.email = "CUSTOMER1@GMAIL.COM";
    reservationDTO.password = "12345";
    const response = await request(app.getHttpServer())
        .post("/reservation")
        .set('Authorization', `Bearer ${accessToken}`)
        .send(reservationDTO)
        ...
        expect(response.body.identification).toBe(reservationDTO.identification);
        expect(response.body.name).toBe(reservationDTO.name);
  });

  it('/reservation (UPDATE)', async () => {
    const reservation = await reservationRepository.save(reservationDB);
    const reservationDTO = new ReservationDTO();
    reservationDTO.identification = "1053847611";
    reservationDTO.name = "Test Name 2";
    reservationDTO.email = "CUSTOMER2@GMAIL.COM";
    reservationDTO.password = "12345";
    const response = await request(app.getHttpServer())
